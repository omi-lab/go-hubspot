...
var openAPILocationPreprocessMap = map[string]map[string]preProcessEntry{
	"CRM": {
		"Crm Associations": {old: "https://api.hubspot.com/api-catalog-public/v1/apis/crm/v4/associations", new: "https://api.hubspot.com/api-catalog-public/v1/apis/crm/v3/associations"},
	},
}
...
func main() {
	...
	for _, group := range r.Results {
		for name, feature := range group.Features {
			g, ok := openAPILocationPreprocessMap[group.Name]
			if ok {
				f, ok := g[name]
				if ok {
					feature.OpenAPI = f.new
				}
			}
